-- Update the Fear & Greed Index function to use CoinMarketCap v3 API
...
COMMENT ON FUNCTION public.update_fear_greed_index() IS 'Fetches the current Fear & Greed Index from CoinMarketCap v3 API with fallback to Alternative.me API and updates the fear_greed_index table. If an entry for today already exists, it will update that entry.'; 
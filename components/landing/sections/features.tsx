      ...
      <div className="container mx-auto px-4">
        ...
        <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
          {features.map((feature, index) => (
            <div
              key={index}
              className="group relative bg-gray-800/40 border border-white/10 rounded-xl p-6 transition-all duration-300 hover:bg-gray-800/60 hover:border-white/20"
            >
              <div className={`absolute top-4 right-4 text-4xl opacity-10 group-hover:opacity-20 transition-opacity duration-300 ${feature.color}`}>
